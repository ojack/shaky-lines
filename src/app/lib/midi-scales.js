const { Scale, Midi, Note} = require('@tonaljs/tonal')

const SCALES = Scale.names()

const notes = Scale.get("A minor pentatonic").notes.map((note) => Midi.toMidi(`${note}3`))

module.exports = (base = "C2", name="pentatonic", numOctaves=1) => {
    const notes = Scale.get(`${base} ${name}`).notes.map((note) => Midi.toMidi(note))
    const transposed = new Array(numOctaves).fill(0).map((_, i) => notes.map((note) => note+12*i)).flat()
    return transposed
}
...
// module.exports = (base, scale, numOctaves) => {
//     const start = base{
